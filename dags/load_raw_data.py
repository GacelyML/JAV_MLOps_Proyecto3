...
def fetch_data(_data_root, _data_filepath):
    # Download data
...
def store_data():
    ...
    df = fetch_data(_data_filepath,_data_filepath)
...
`glyburide_metformin` VARCHAR(50), `glipizide_metformin` VARCHAR(50), `glimepiride_pioglitazone` VARCHAR(50), `metformin_rosiglitazone` VARCHAR(50), 
`metformin_pioglitazone` VARCHAR(50), `change` VARCHAR(50), `diabetesMed` VARCHAR(50), `readmitted` VARCHAR(50))"""
    ...
    cur.execute("""
        ...
        INTO TABLE `{}`
        IGNORE 1 LINES
        """.format(_data_filepath, 'raw_diabetes'))
    conn.commit()
    return 'raw_diabetes'
